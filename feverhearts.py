...
def main():
    ...
    while x not in menudict:
        ...
        if menudict[x] == 3:
            print("Welcome to Fever Hearts.")
            print("The goal of game is to form a path of open valves (denoted by letters)")
            print("from one end (the 'hot' red heart) to the other (the 'cold' blue heart),")
            print("as connected by the veins (denoted by [/], [\\] or [-]).")
            print("All valves are closed for a set number of turns, and then open for")
            print("a set, smaller number of turns. These values are affected by 'temperature'.")
            print("Every object has a temperature: hot, warm, lukewarm, or cold.")
            print("Their symbol is colored to demonstrate this state.")
            print("Temperature only affects valves. Whenever a valve opens,")
            print("the temperature between it, all veins connected to it,")
            print("all open other valves connected to those veins, and so on,")
            print("are averaged. If a heart is connected in this way,")
            print("its value contributes to the average, but is not changed.")
            print("The colder a valve is, the longer it stays closed,")
            print("and the shorter it remains open.")
            print("Above each valve is an X (denoting closed) or O (denoting open).")
            print("Below each valve is a number, denoting how many more turns it will")
            print("remain closed/open. After every action, every valve's counter is reduced by one.")
            print("To play, simply type in the letter (capital or lowercase)")
            print("of the valve you would like to target. If that valve is closed,")
            print("its counter will be reduced by one further, effectively by two.")
            print("If that valve is open, then the counter will be increased by one, effectively by zero.")
